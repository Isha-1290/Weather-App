...
from API.services.DBManipulation import commit_to_db
...
def format_response(weather_json):
    try:
        ...
        final_str = 'City: %s \nCountry: %s \nConditions: %s \nTemperature (°F): %s\nHumidity: %s\nWind Speed(km/h): %s' % (city, country, conditions, temp, humidity, (speed*3.6))
    except:
...
background_image= tk.PhotoImage(file='./API/static/pic.png')
background_label = tk.Label(app, image=background_image)
background_label.place(x=0, y=0, relwidth=1, relheight=1)

C.pack()

frame = tk.Frame(app,  bg='#e36414', bd=5)
...
lower_frame = tk.Frame(app, bg='#e36414', bd=10)
